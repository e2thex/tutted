...
var tuttedStdMarkdownRender = function(tree, renders) {
  ...
  var markdown = [];
  tree.getModules().forEach(function(branch) {
    markdown = markdown.concat(renders.module(branch, 1));
  });
...
tuttedStdMarkdownRender.throw = function tuttedStMarkdownRenderThrow(tree) {
  return ["| " + tree.getType() + " | " + tree.getDesc().join(" ") + " |"];
}
...
tuttedStdMarkdownRender.function = function tuttedStMarkdownRenderFunction(tree, level, renders) {
  ...
  var throwsBranch = tree.getThrows();
  if(throwsBranch.length > 0) {
    markdown.push(heading(level+1) + " Throws");
    markdown.push("| Type | Desc |");
    markdown.push("|:---- |:---- |");
    throwsBranch.forEach(function(throwBranch) {
      markdown.push(renders.throw(throwBranch)[0]);
    }, this);
  }
